...
def _write_overview() -> None:
    ...
    try:
        ...
        body = (
            f"<!-- Generated by mimir — {ts}. Stale or wrong? Call "
            f'mimir("status"), or run `mimir sync` to force a refresh. -->\n\n'
...
_MIMIR_SECTION_MARKER = f"{_MIMIR_MARKER_PREFIX} (v2)"
...
def setup() -> None:
        ...
        if not is_global:
            ...
            if not overview_path.exists():
                overview_path.write_text(
                    ...
                    "server starts in this workspace. Still a placeholder? Call "
                    'mimir("status"), or run `mimir sync`. -->\n',
                    encoding="utf-8",
                )
    ...
    if arg == "copilot-cli":
        ...
        copilot_snippet = (
            ...
            "mimir(\"set_scope\", dir) to hard-narrow (excludes other sub-projects entirely) or "
    ...
    if mimirignore.exists():
        ...
        mimirignore.write_text(
            ...
            "# Use mimir(\"ignore\", \"pattern, reason\") in Claude/Copilot to add patterns on the fly.\n"
            "\n"
...
MCP TOOLS (available to Claude Code and GitHub Copilot)
  ...
  mimir     Everything else — status, arch, changed, set_focus, set_scope, alias,
            note, ignore, audit, help — plus every pre-v2 tool name (scope_task,
            get_symbol, find_callers, ...) for instructions generated before this
            3-tool redesign. Call mimir("help") for the full command list.

  (Prior to 2026-07-09, mimir registered 18 separate MCP tools — get_status,
  get_symbol, scope_task, etc. Those are all still reachable as mimir(...) commands;
  see mimir("help", "<command>") for each one's original argument docs.)
...
DOMAIN ALIASES
  Create .mimiraliases in the project root to map feature names to code names:
    corrective actions = RectificationFilter
    live tutor = LiveTutor, GeminiLive
  Or let Claude/Copilot call mimir("alias", "domain term, CodeName") to build it
...
def _cli_run(subcommand: str, arg: str) -> None:
    ...
    if not _FTS_READY:
        ...
        if subcommand in ("scope", "area", "locate"):
            # scope_task/scope_area/locate's ranking needs these in-memory structures.
...
def _cmd_help(args: str) -> str:
    ...
    if not name:
        return f"# mimir commands\n\n{_COMMAND_LIST}\n\nUse mimir(\"help\", \"<command>\") for one command's full argument docs."
    entry = _COMMANDS.get(name)
    if entry is None:
        return f"Unknown command {name!r}. Call mimir(\"help\") for the list."
...
def mimir_dispatch(command: str, args: str = "") -> str:
    ...
    cover discovery and reading; this covers the rest. Call
    mimir("help") for the full command list, or mimir("help", "<command>")
    for one command's exact argument format.
...
# Only these three are registered as MCP tools (see _main below) — everything
# else in _TOOLS (populated by @_tool above) stays a plain, directly callable
# Python function used by the CLI and by mimir_dispatch's legacy routing.
...
def _main() -> None:
    ...
    for fn in _MCP_TOOLS:
        mcp.tool()(fn)
    mcp.tool(name="mimir")(mimir_dispatch)
