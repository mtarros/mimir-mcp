...
_OVERVIEW_FILE = ".mimir-overview.md"
_LAST_OVERVIEW_HASH: Optional[str] = None
_OVERVIEW_DEBOUNCE_TIMER: Optional["threading.Timer"] = None
_OVERVIEW_DEBOUNCE_LOCK = threading.Lock()


def _write_overview() -> None:
    """Write a session-orientation snapshot (architecture map + index stats +
    active scope/focus + timestamp) to .mimir-overview.md so a generated
    CLAUDE.md can @-import it — session orientation with zero tool calls,
    instead of a get_architecture()/get_status() round trip at the start of
    every session.

    Hash-guarded on the STABLE content (architecture map + stats + scope/
    focus, not the timestamp): skips the write, and leaves the existing
    timestamp alone, when nothing actually changed — otherwise every
    warmup/watcher firing would touch the file's mtime for no reason.
    Best-effort: never raises, since this must not be able to break warmup
    or the file watcher.
    """
    global _LAST_OVERVIEW_HASH
    try:
        arch = get_architecture()
        n_files = len(_iter_source_files())
        scope_line = _ACTIVE_SCOPE or "none (whole repo)"
        focus_line = ", ".join(f"{k}:{v}" for k, v in _FOCUS_WEIGHTS.items()) or "none"
        stable = f"{arch}\n---\nfiles={n_files} scope={scope_line} focus={focus_line}"
        digest = hashlib.sha1(stable.encode("utf-8")).hexdigest()
        if digest == _LAST_OVERVIEW_HASH:
            return
        _LAST_OVERVIEW_HASH = digest
        ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        body = (
            f"<!-- Generated by mimir — {ts}. Stale or wrong? Run "
            f'`mimir status`, or `mimir sync` to force a refresh. -->\n\n'
            f"# Workspace overview\n\n"
            f"Source files indexed: {n_files}\n"
            f"Active scope: {scope_line}\n"
            f"Active focus: {focus_line}\n\n"
            f"{arch}\n"
        )
        (WORKSPACE_ROOT / _OVERVIEW_FILE).write_text(body, encoding="utf-8")
    except Exception:
        pass


def _schedule_overview_write(delay: float = 5.0) -> None:
    """Debounced trigger for _write_overview() from the file watcher — one
    file event during a burst of saves (branch switch, bulk refactor) fires
    this repeatedly; only the last one within `delay` seconds of quiet
    actually regenerates the file, so a 200-file branch switch costs one
    architecture rebuild, not 200."""
    global _OVERVIEW_DEBOUNCE_TIMER
    with _OVERVIEW_DEBOUNCE_LOCK:
        if _OVERVIEW_DEBOUNCE_TIMER is not None:
            _OVERVIEW_DEBOUNCE_TIMER.cancel()
        _OVERVIEW_DEBOUNCE_TIMER = threading.Timer(delay, _write_overview)
        _OVERVIEW_DEBOUNCE_TIMER.daemon = True
        _OVERVIEW_DEBOUNCE_TIMER.start()
...
_MIMIR_MARKER_PREFIX = "## Code exploration — use mimir tools"
_MIMIR_SECTION_MARKER = f"{_MIMIR_MARKER_PREFIX} (v3)"


def _upsert_mimir_section(path: Path, new_section: str) -> str:
    """Create, replace, or skip mimir's generated section in a CLAUDE.md /
    copilot-instructions.md file. Returns "created", "updated", or "skipped".

    Matches ANY past mimir section by its version-less marker prefix, not
    just the current one — so re-running mimir-setup after a tool-surface
    change like this one (18 tools -> locate/inspect/mimir) actually
    upgrades a stale section instead of leaving it alone forever. The old
    behavior ("skip if the marker is present at all") meant a file written
    before this redesign would keep telling the agent to call tool names
    that no longer exist, with no way to fix it short of manual editing.
    """
    text = path.read_text(encoding="utf-8") if path.exists() else ""
    if _MIMIR_SECTION_MARKER in text:
        return "skipped"
    idx = text.find(_MIMIR_MARKER_PREFIX)
    if idx == -1:
        sep = "" if not text else ("\n" if text.endswith("\n") else "\n\n")
        path.write_text(text + sep + new_section, encoding="utf-8")
        return "created" if not text else "updated"
    # Replace the whole stale section: from its marker line up to the next
    # top-level heading, or end of file.
    tail = text[idx:]
    cut = len(tail)
    for m in re.finditer(r'^#{1,2} ', tail, re.MULTILINE):
        if m.start() > 0:
            cut = m.start()
            break
    before = text[:idx].rstrip("\n")
    after = tail[cut:].lstrip("\n")
    new_text = before + ("\n\n" if before else "") + new_section.rstrip("\n") + "\n\n" + after
...
def setup() -> None:
    ...
    if arg == "claude":
        ...
        overview_line = "" if is_global else "@.mimir-overview.md\n\n"
        snippet = (
            f"{_MIMIR_SECTION_MARKER}\n\n"
        ...
        action = _upsert_mimir_section(claude_md, snippet)
        label = "mimir section already present" if action == "skipped" else f"mimir section {action}"
        print(f"{action}  {claude_md}  ({label})")
        if not is_global:
            overview_path = cwd / _OVERVIEW_FILE
            if not overview_path.exists():
                overview_path.write_text(
                    "<!-- Placeholder — populated automatically the first time the mimir MCP "
                    "server starts in this workspace. Still a placeholder? Run "
                    "`mimir status`, or `mimir sync`. -->\n",
                    encoding="utf-8",
                )
                print(f"created  {overview_path}  (placeholder — filled in on first server start)")
    ...
    if arg == "copilot-cli":
        ...
        copilot_snippet = (
            ...
            "\"the mobile app\", \"the API\"), run `mimir set_focus \"matching-prefix:3\"` "
            "immediately, or pass focus=\"prefix:3\" directly on individual locate() calls. In a "
            "multi-sub-project repo, unscoped ranking silently defaults to whichever sub-project has "
            "the most indexed symbols; it will NOT reliably surface the right area on its own\n"
            ...
            "call — much cheaper than reading the whole file, and cheaper than one inspect() call "
            "per symbol\n"
            ...
            "`mimir alias \"domain term, CodeName\"` to save it — future locate() searches expand "
            "it automatically\n"
            ...
            "locate(task, mode=\"area\") to see where matches cluster, then "
            "`mimir scope --set <dir>` to hard-narrow (excludes other sub-projects entirely) or "
        ...
        action = _upsert_mimir_section(copilot_instructions, copilot_snippet)
        label = "mimir section already present" if action == "skipped" else f"mimir section {action}"
        print(f"{action}  {copilot_instructions}  ({label})")
    ...
    if mimirignore.exists():
        ...
        mimirignore.write_text(
            ...
            "# Use `mimir ignore \"pattern, reason\"` (run via the shell) to add patterns on the fly.\n"
            "\n"
...
TERMINAL COMMANDS
  ...
  mimir locate "<task>"    Find files/symbols relevant to a plain-English task
                            description — the CLI form of the locate MCP tool
  mimir scope  "<task>"    Same search, in scope_task's original (verbose) format
  ...
  mimir sync               Force-regenerate .mimir-overview.md (normally automatic —
                            written at server startup and debounced on file changes)
  mimir <command> "<args>" Everything else not listed above: arch, changed, set_focus,
                            set_scope, alias, note, ignore, plus every pre-v2 tool name
                            (scope_task, get_symbol, find_callers, ...) — run
                            `mimir help` for the full list, `mimir help <command>` for
                            one command's exact original argument docs
  mimir --help             Show this help

EXAMPLES
  mimir hint   "quiet zone notification volume"
  mimir locate "change how jobs are retried on failure"
...
MCP TOOLS (available to Claude Code and GitHub Copilot)
  locate    Find files/symbols relevant to a plain-English task description — call
            first for any existing-code task. mode="area"/"symbol" cover scope_area/
            verify_symbol_existence too.
  inspect   Read code: a file's structure, one or more symbol bodies (comma-separate
            names to batch), or view="imports"/"dependents"/"callers" for a file/symbol.

  Only these two are registered — everything else (status, arch, changed, set_focus,
  set_scope, alias, note, ignore, audit, help, and every pre-v2 tool name) is a
  `mimir <command> "<args>"` shell command instead (see TERMINAL COMMANDS below and
  `mimir help`), not a third registered tool: those are occasional/session-level
  actions, not needed every task, so keeping them off the wire schema costs nothing
  in practice and saves real per-turn overhead.

  (Prior to 2026-07-09, mimir registered 18 separate MCP tools — get_status,
  get_symbol, scope_task, etc; before that, all 18 collapsed into ONE extra
  registered "mimir" dispatcher tool. Both generations' tool names are all still
  reachable as `mimir <name> "<args>"` shell commands; run `mimir help <name>` for
  each one's original argument docs.)
...
EXCLUDING FILES
  ...
  - Ask Claude/Copilot to run `mimir ignore "pattern, reason"` when it spots noisy files
...
DOMAIN ALIASES
  Create .mimiraliases in the project root to map feature names to code names:
    corrective actions = RectificationFilter
    live tutor = LiveTutor, GeminiLive
  Or let Claude/Copilot run `mimir alias "domain term, CodeName"` to build it
  automatically. Mimir applies aliases before every locate/scope_task search.
...
def _cli_run(subcommand: str, arg: str) -> None:
    ...
    if not _FTS_READY:
        ...
        # scope/area/locate/hint's ranking, and most of the mimir_dispatch
        # fallback's read-oriented legacy commands (scope_task, scope_hint,
        # semantic_search, get_dependents, ...), need these in-memory
        # structures. _build_token_df/_build_path_strings aren't disk-
        # persisted so they rebuild every process regardless (cheap — SQL
        # query + blueprint scan). _REVERSE_IMPORTS IS disk-persisted
        # (reverse_imports table) once a previous full _warm_cache() has
        # run — try that first to avoid re-reading the first 8KB of every
        # JS/TS/Python/Java/Kotlin file on every single CLI invocation.
        # Skipped only for the handful of subcommands that provably never
        # touch ranking (pure file-writes, symbol-index-only lookups, or
        # diagnostics) — building it for everything else is deliberately
        # generous rather than trying to enumerate every fallback command
        # precisely, since the cost is already described as cheap.
        _CLI_CHEAP_NO_RANKING = {
            "find", "callers", "status", "audit", "sync", "help",
            "set_focus", "set_scope", "alias", "note", "ignore",
        }
        if subcommand not in _CLI_CHEAP_NO_RANKING:
    ...
    if subcommand == "hint":
        ...
        _write_overview()
        print(f"[mimir] wrote {WORKSPACE_ROOT / _OVERVIEW_FILE}")
    else:
        # Fallback for everything mimir_dispatch handles that doesn't have its
        # own dedicated subcommand above: status/arch/changed/set_focus/
        # set_scope/alias/note/ignore/audit/help, plus every pre-v2 tool name
        # (scope_task, get_symbol, find_callers, ...). This is the ONLY way
        # to reach those from the CLI since mimir_dispatch isn't MCP-
        # registered. mimir_dispatch reports its own "Unknown command" for
        # anything it doesn't recognize either — detect that specific case to
        # preserve a non-zero exit code for scripting, same as before.
        result = mimir_dispatch(subcommand, arg)
        print(result)
...
# =============================================================================
# v2 tool surface — locate / inspect / mimir
#
# Registered MCP tools: 18 -> 3 (2026-07-09 redesign, see docs/plan-v2-tool-
# redesign.md). Every plain function above (scope_task, get_symbol, etc.) is
# untouched and still directly callable — only what main() registers with
# FastMCP shrinks, cutting the per-turn tool-schema wire cost from ~15,100 to
# under 4,000 chars. locate/inspect cover the hot path (discovery + reading)
# as real typed tools; mimir_dispatch covers everything else, including every
...
def _render_locate(task: str, max_files: int, focus: str) -> Optional[str]:
    ...
    scope_task's separate 20-line Matched-symbols section, plus one batched
    inspect() suggestion per top file. Returns None when the exact-token
    symbol index has nothing, so the caller can fall through to
    semantic_search instead of returning an empty result.
    """
    file_hit_count, keywords, all_hits, expanded, used_fts = _score_task_files(task, focus)
    if not keywords or not file_hit_count:
        return None
    ...
    # Same low-IDF suppression scope_task uses, so a hit whose only signal is
    # a ubiquitous term (or the active scope's own name) doesn't inflate a
    # file's symbol list or generate a useless inspect() suggestion.
    relevant = [(kw, r, l, s) for kw, r, l, s in all_hits if r in top_set]
    low_idf = {kw for kw in {h[0] for h in relevant} if _idf_weight_in_scope(kw) < 0.25}

    file_syms: dict[str, list[str]] = {}
    file_syms_seen: dict[str, set[str]] = {}
    file_kws: dict[str, set[str]] = {}
    for kw, r, l, s in relevant:
        file_kws.setdefault(r, set()).add(kw)
        if not _DEF_LINE_PAT.search(s):
            continue
        name = _symbol_name_from_sig(s)
        if not name:
            continue
        seen = file_syms_seen.setdefault(r, set())
        if name not in seen:
            seen.add(name)
            file_syms.setdefault(r, []).append(name)

    parts = [f"# locate: {task!r}  ({len(file_hit_count)} matched, showing {len(top_files)})"]
    if expanded != task:
    ...
    for i, rel in enumerate(top_files, 1):
        ...
        if syms:
            ...
            sym_part = f"  [{stem}: {', '.join(shown)}{tail}]"
            suggestions.append(f'inspect("{rel}", "{",".join(syms[:6])}")')
        else:
            sym_part = ""
        ...
        for rel in top_files:
            for dep in _REVERSE_IMPORTS.get(rel, []):
                if dep not in top_set:
                    dependent_set.add(dep)
        if 0 < len(dependent_set) <= 8:
            shown_deps = sorted(dependent_set)[:4]
            extra = len(dependent_set) - len(shown_deps)
            tail = f", +{extra}" if extra else ""
            parts.append(f"Dependents: {', '.join(shown_deps)}{tail}")

    return "\n".join(parts)


def locate(task: str, max_files: int = 5, focus: str = "", mode: str = "files") -> str:
    """Find files/symbols relevant to a plain-English task — call FIRST for
    any existing-code task, before reading files or grepping. Then call
    inspect() on the symbols it suggests. Replaces scope_task/scope_hint/
    scope_area/semantic_search/verify_symbol_existence. Falls through to a
    semantic search automatically when exact matching finds nothing.

    TIP: prefer class/function names when known over domain language — the
    index matches code names, not feature names.

    Args:
        task: what you want to do, or (mode="symbol") a bare identifier.
        max_files: files to rank and return (default 5).
        focus: optional "prefix:weight" override for this call only.
        mode: "files" (default), "area" (directory-tree view, for
            monorepos), or "symbol" (confirm one identifier is defined).
    """
    try:
        mode = (mode or "files").strip().lower()
        if mode == "area":
            return scope_area(task, focus=focus)
        if mode == "symbol":
            return verify_symbol_existence(task.strip())
        if mode != "files":
            return f"Error: unknown mode {mode!r}. Expected: files, area, symbol."

        max_files = max(1, min(int(max_files), 25))
        rendered = _render_locate(task, max_files, focus)
        if rendered is not None:
            return rendered
        fallback = semantic_search(task, max_results=max_files, focus=focus)
        return "[locate: no exact-symbol matches — falling back to semantic search]\n" + fallback
    except Exception as e:
        return f"Error in locate: {type(e).__name__}: {e}"


def inspect(path: str = "", symbol: str = "", view: str = "auto") -> str:
    """Read code — replaces get_symbol/get_file_structure/get_imports/
    get_dependents/find_callers with one entry point. Pass `symbol` (one
    name, or several comma-separated) to read just those bodies in full;
    omit it to get the file's (or directory's) structural blueprint instead;
    or set `view` to look at a file from a different angle.

    Args:
        path: workspace-relative file or directory path. Not required when
            view="callers".
        symbol: one symbol name, or several comma-separated (cheaper than
            one inspect() call per name — one shared header instead of
            several), to read in full instead of just the structure.
        view: "auto" (default — structure or symbol body per the above),
            "imports" (resolve this file's imports), "dependents" (files
            that import this file), or "callers" (every call/usage site of
    ...
    try:
        ...
        if view == "callers":
            if not symbol.strip():
                return 'Error: view="callers" requires `symbol` (the identifier to search for).'
            return find_callers(symbol.strip())
        if not path.strip():
            return 'Error: `path` is required unless view="callers".'
        if view == "imports":
            return get_imports(path)
        if view == "dependents":
            return get_dependents(path)
        if view != "auto":
            return f"Error: unknown view {view!r}. Expected: auto, imports, dependents, callers."
        if symbol.strip():
            return get_symbol(path, symbol)
        return get_file_structure(path)
    except Exception as e:
        return f"Error in inspect: {type(e).__name__}: {e}"


# --- mimir_dispatch: everything else, plus every pre-v2 tool name ----------

_KV_RE = re.compile(r'(?:^|\s)(\w+)=(\S+)')


def _extract_kwargs(args: str) -> tuple[str, dict[str, str]]:
    """Pull trailing `key=value` tokens out of a dispatcher args string,
    returning the leftover free-text plus the parsed kwargs. Values must be
    whitespace-free (numbers, bools, "prefix:weight[,prefix:weight...]"
    strings all qualify — a comma inside a value is fine, only whitespace
    ends it)."""
    kwargs: dict[str, str] = {}
    ...
    remaining = _KV_RE.sub(_grab, args)
    return remaining.strip(), kwargs


def _dispatch_scope_task(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return scope_task(
        text,
        max_files=int(kw.get("max_files", 5)),
        include_blueprints=kw.get("include_blueprints", "").strip().lower() == "true",
        focus=kw.get("focus", ""),
    )


def _dispatch_scope_hint(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    return scope_hint(text)


def _dispatch_scope_area(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return scope_area(text, max_depth=int(kw.get("max_depth", 4)), focus=kw.get("focus", ""))


def _dispatch_semantic_search(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return semantic_search(text, max_results=int(kw.get("max_results", 10)), focus=kw.get("focus", ""))


def _dispatch_verify_symbol(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return verify_symbol_existence(text, max_results=int(kw.get("max_results", 25)), path=kw.get("path", ""))


def _dispatch_get_symbol(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    path, _, symbol_name = text.partition(",")
    return get_symbol(path.strip(), symbol_name.strip())


def _dispatch_get_file_structure(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return get_file_structure(text, max_files=int(kw.get("max_files", 10)))


def _dispatch_get_imports(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    return get_imports(text)


def _dispatch_get_dependents(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    return get_dependents(text)


def _dispatch_find_callers(args: str) -> str:
    text, kw = _extract_kwargs(args)
    return find_callers(text, max_results=int(kw.get("max_results", 20)))


def _dispatch_get_status(_args: str) -> str:
    return get_status()


def _dispatch_get_architecture(_args: str) -> str:
    return get_architecture()


def _dispatch_get_changed_files(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    return get_changed_files(text.strip() or "main")


def _dispatch_set_focus(args: str) -> str:
    text, kw = _extract_kwargs(args)
    persist = kw.get("persist", "true").strip().lower() != "false"
    return set_focus(text, persist=persist)


def _dispatch_set_scope(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    return set_scope(text)


def _dispatch_record_alias(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    domain_term, _, code_name = text.partition(",")
    return record_alias(domain_term.strip(), code_name.strip())


def _dispatch_record_note(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    path_prefix, _, note = text.partition(",")
    return record_note(path_prefix.strip(), note.strip())


def _dispatch_add_ignore(args: str) -> str:
    text, _kw = _extract_kwargs(args)
    pattern, _, reason = text.partition(",")
    return add_ignore(pattern.strip(), reason.strip())


def _dispatch_audit(_args: str) -> str:
    return audit_index_health()


# name -> (dispatch wrapper, real function whose docstring documents the
# original args — used by `mimir help <command>`).
_COMMANDS: dict[str, tuple[Callable[[str], str], Callable]] = {
    "status": (_dispatch_get_status, get_status),
    "arch": (_dispatch_get_architecture, get_architecture),
    "changed": (_dispatch_get_changed_files, get_changed_files),
    "set_focus": (_dispatch_set_focus, set_focus),
    "set_scope": (_dispatch_set_scope, set_scope),
    "alias": (_dispatch_record_alias, record_alias),
    "note": (_dispatch_record_note, record_note),
    "ignore": (_dispatch_add_ignore, add_ignore),
    "audit": (_dispatch_audit, audit_index_health),
    # Legacy pre-v2 tool names — unchanged behavior, for old generated
    # instructions/muscle memory. "scope" (not "set_scope") is the CLI's
    # long-standing name for the scope_task query, so it's aliased here too.
    "scope_task": (_dispatch_scope_task, scope_task),
    "scope": (_dispatch_scope_task, scope_task),
    "scope_hint": (_dispatch_scope_hint, scope_hint),
    "scope_area": (_dispatch_scope_area, scope_area),
    "semantic_search": (_dispatch_semantic_search, semantic_search),
    "verify_symbol_existence": (_dispatch_verify_symbol, verify_symbol_existence),
    "get_symbol": (_dispatch_get_symbol, get_symbol),
    "get_file_structure": (_dispatch_get_file_structure, get_file_structure),
    "get_imports": (_dispatch_get_imports, get_imports),
    "get_dependents": (_dispatch_get_dependents, get_dependents),
    "find_callers": (_dispatch_find_callers, find_callers),
    "get_status": (_dispatch_get_status, get_status),
    "get_architecture": (_dispatch_get_architecture, get_architecture),
    "get_changed_files": (_dispatch_get_changed_files, get_changed_files),
    "record_alias": (_dispatch_record_alias, record_alias),
    "record_note": (_dispatch_record_note, record_note),
    "add_ignore": (_dispatch_add_ignore, add_ignore),
    "audit_index_health": (_dispatch_audit, audit_index_health),
}

_COMMAND_LIST = (
    "status, arch, changed, set_focus, set_scope, alias, note, ignore, audit, help — "
    "plus every pre-v2 tool name: scope_task, scope_hint, scope_area, semantic_search, "
    "verify_symbol_existence, get_symbol, get_file_structure, get_imports, get_dependents, "
    "find_callers, get_status, get_architecture, get_changed_files, record_alias, "
    "record_note, add_ignore, audit_index_health."
)
...
def _cmd_help(args: str) -> str:
    ...
    if not name:
        return f"# mimir commands\n\n{_COMMAND_LIST}\n\nRun `mimir help <command>` for one command's full argument docs."
    entry = _COMMANDS.get(name)
    if entry is None:
        return f"Unknown command {name!r}. Run `mimir help` for the list."
    _wrapper, target = entry
    doc = (target.__doc__ or "(no docs)").strip()
    return f"# {name}\n\n{doc}"


def mimir_dispatch(command: str, args: str = "") -> str:
    """Session/workspace management, plus every pre-v2 tool name (for
    instructions generated before this tool was added) — locate and inspect
    cover discovery and reading; this covers the rest. NOT an MCP tool as of
    2026-07-09 (see _MCP_TOOLS) — reachable only via the CLI: `mimir <command>
    "<args>"`. Run `mimir help` for the full command list, or
    `mimir help <command>` for one command's exact argument format.

    Args:
        command: status, arch, changed, set_focus, set_scope, alias, note,
            ignore, audit, help — or any pre-v2 tool name (scope_task,
            get_symbol, find_callers, etc.).
        args: command-specific free text. Optional typed parameters can be
            appended as trailing key=value tokens, e.g. "font icon rendering
            max_files=8 focus=Keypad:3". `mimir help <command>` shows the
            exact original argument order for each command.
    """
    ...
    wrapper, _target = entry
    try:
        return wrapper(args or "")
    except Exception as e:
        return f"Error running '{cmd}': {type(e).__name__}: {e}"


# Only these two are registered as MCP tools (see _main below). mimir_dispatch
# (the CLI/legacy-name catch-all previously registered as the "mimir" tool)
# was pulled from MCP registration on 2026-07-09: its commands (status, arch,
# alias, note, ignore, set_focus, set_scope, and every pre-v2 tool name) are
# occasional/session-level actions, not called on every task the way
# locate/inspect are — moving it to CLI-only cut another ~30% of wire size
# with much less adoption risk than doing the same to locate/inspect, which
# are used constantly and need the reliability of being a real registered
# tool. mimir_dispatch is still reachable from an AI client via the CLI
# (`mimir <command> "<args>"` through the shell/Bash tool) — see _cli_run's
# fallback branch. Everything else in _TOOLS (populated by @_tool above)
# stays a plain, directly callable Python function too.
_MCP_TOOLS: list = [locate, inspect]
...
def _main() -> None:
    ...
    _CLI_ARG_HINT = {
        ...
        "find": "SymbolName",
        "callers": "SymbolName",
        "locate": "task description",
    }
    _CLI_NO_ARG = ("status", "audit", "sync")
    if args and args[0] in (*_CLI_ARG_HINT, *_CLI_NO_ARG):
        subcommand = args[0]
        arg = " ".join(args[1:]) if len(args) > 1 else ""
        if subcommand not in _CLI_NO_ARG and not arg:
            print(f"Usage: mimir {subcommand} <{_CLI_ARG_HINT[subcommand]}>")
            sys.exit(1)
        _cli_run(subcommand, arg)
        return

    # Bare `mimir -h`/`--help`/`help` (no further args) prints the full
    # static CLI reference below. `mimir help <command>` (an extra arg
    # present) instead falls through to the generic dispatch just below,
    # which routes to mimir_dispatch("help", "<command>") for that one
    # command's real docstring — more useful than repeating the whole page.
    if args and args[0] in ("-h", "--help") or args == ["help"]:
        print(_CLI_HELP)
        return

    if args:
        # Generic fallback: mimir_dispatch was pulled from MCP registration
        # (2026-07-09, see _MCP_TOOLS) since its commands are occasional/
        # session-level rather than needed every task — the CLI is now the
        # only way an AI client reaches status/arch/changed/set_focus/
        # set_scope/alias/note/ignore/audit/help/<command>, and every pre-v2
        # tool name (scope_task, get_symbol, find_callers, ...). Genuinely
        # unrecognized input still gets mimir_dispatch's own self-teaching
        # "Unknown command" response, not a silent no-op.
        subcommand = args[0]
        arg = " ".join(args[1:]) if len(args) > 1 else ""
        _cli_run(subcommand, arg)
        return
