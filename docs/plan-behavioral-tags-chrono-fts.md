...
## Investigated and reverted: Virtual Token Expansion / concept synonym expansion

**What it would have done**: a ticket saying "save the record" would also search "persist"/"store"/"write" — the code may spell the concept differently. The original proposal's naive version (each synonym added as an independent keyword) had a real scoring flaw, confirmed against actual code during the initial ground-truthing pass: `_score_task_files`'s `_KW_CAP`/IDF accounting is keyed by exact keyword string, so a file weakly matching several synonyms could stack independent caps and out-score a file precisely matching the literal term once. A corrected version was built (every synonym in a matched group routed to a shared `scoring_key`/cap, with two related bugs found and fixed along the way — a keyword-diversity-multiplier double-count, and a short-word extraction gap), unit-tested (8 tests), and A/B'd twice against real Carps-git tickets.

**Result: a wash, not a win — reverted in full.** First version (8 concept groups): only 13/60 real ticket queries even contained a trigger word; of those, 1 improved, 2 regressed, MRR unchanged (0.340→0.340). A narrowed 6-group version (dropped "update"/"validate," both common enough to match incidentally in doc/config prose): 8/60 triggered, 1 improved, 1 regressed, **MRR 0.340→0.342** — statistically indistinguishable from no effect, not a real improvement. This was initially shipped anyway on the reasoning that it was harmless with some standalone correctness value (the anti-stacking fix). **User pushed back explicitly: ship only verified wins, not washes** — the feature was fully reverted (code, tests, this doc) rather than kept. See `CLAUDE.md`'s "Ship-only-verified-wins rule" for the standing policy this established.

**Why this is different from the 3 features that DID ship** (`#tags`, `chrono_fts`, `EXT_LANG` file-type coverage): those three each closed a real, mechanical gap (hidden framework usage, no commit-message search, unindexed file types) that was structurally absent before. Synonym expansion instead has to win a *popularity contest* against existing signal on real tickets — most real ticket titles already use language close enough to the actual code (or get picked up by chrono_fts/existing keyword matching) that a generic English synonym map has few clean opportunities to help, and every opportunity to help is matched by an opportunity to introduce incidental noise. A legitimate, real negative result, not a failed implementation — just a different, harder kind of improvement lever than the other three, and this repo's real ticket language didn't have much of the specific gap (user says one verb, code uses a genuinely different one) this feature targets. If revisited, it needs a real net-positive A/B before merging, not just a plausible mechanism and "no harm."

## Investigated and reverted: adjacent-short-word joining (phrasal verbs)

Found via a real miss: `"Can't log out of device"` produces zero keyword signal about "logout" — `_extract_scope_keywords`'s plain-word rule requires 5+ chars, and "log"/"out" are both too short. Manually appending "logout" to the query surfaced several genuinely relevant Logout-related files that weren't visible before. Built a mechanical (not curated) extension of the existing capitalized-bigram-joining logic: join any two adjacent short (2-4 char) words and let it flow through the same "no hits = no cost" gate the capitalized version already relies on — no synonym list to get wrong, unlike the reverted concept-synonym idea.

**Result: a wash on both real repos, reverted.** The isolated real-world test on the exact "log out" query DID surface new, genuinely relevant files (`LogoutPage.xaml.cs`, logout-related test files) that weren't visible before — the mechanism works. But across the full 60-task real samples on both Carps-git and top-cat, the miss list and aggregate metrics were statistically unchanged (Carps: 0.418→0.415, top-cat: 0.310→0.310) — other adjacent short-word pairs in typical ticket prose diluted the `_compounds[:4]` cap before the useful join could win a slot, and even where it worked, it didn't happen to surface the specific historically-recorded ground-truth file for that ticket. Reverted cleanly (zero net diff on `mimir.py`) rather than kept on the strength of one anecdote. A real, informative negative result — the general lesson from this session holds: a demonstrably-working mechanism on one hand-picked example is not evidence of a real win until it's measured on a full real sample.

## Follow-up fine-tuning pass (same session): 3 recommendations tested, 1 built, 1 reverted, 1 delivered as a workflow artifact

After Features 1-3 shipped, the user asked for further recommendations and to "work on all of them." Four items were investigated:
